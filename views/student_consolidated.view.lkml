   ...
   derived_table: {
     sql: select y.* from
            (select ss.* from stars.student_snapshot ss
               ...
               inner join (select s.school_year, s.student_id, max(s.period_key) max_period_key from stars.student_snapshot s inner join stars.period p on p.period_key=s.period_key and p.period_level in ('40D', '80D', '120D')
